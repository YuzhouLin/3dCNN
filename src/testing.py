...
def test_all(cfg):
    ...
    day_list = cfg.DATA_CONFIG.day_list+cfg.DATA_CONFIG.day_test_list
    for day_n in day_list:
        print('on day: ', day_n)

        for time_n in cfg.DATA_CONFIG.time_list:
            for folder in ["train", "val", "test"]:
                if (day_n not in cfg.DATA_CONFIG.day_test_list and folder == "test") or (day_n in cfg.DATA_CONFIG.day_test_list and folder!="test"):
                    continue
                test_X = np.load(cfg.DATA_PATH+f's{sb_n}/{folder}/X_d{day_n}_t{time_n}.npy')
                Y_numpy = np.load(cfg.DATA_PATH+f's{sb_n}/{folder}/Y_d{day_n}_t{time_n}.npy')

                X_torch = torch.from_numpy(np.array(test_X, dtype=np.float32))
                Y_torch = torch.from_numpy(Y_numpy)
