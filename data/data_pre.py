...
def prepare_data(data_params):
    fs = data_params['fs']
    wl = round(data_params['ws']/1000*fs) # window length
    non_overlap_n = round(wl*data_params['ratio_non_overlap'])
    rest_discard_n = round(0.5*fs) # the number of discard samples for rest after each gesture 
        ...
        for day_n in data_params['day_list']:
            for time_n in data_params['time_list']:
                ...
                for k in data_params['trial_list']:
                    ...
                    for i, m in enumerate(data_params['class_list']):
                        index = np.nonzero(np.logical_and(cycle==k, label==m))[0]
                        samples = emg[index][:,valid_channel]
                    
                        # segment data for each gesture
                        temp = []
                        for n in range(0,samples.shape[0]-wl, non_overlap_n):
                            # segdata = samples[n:n+wl,:] # 500*14
                            segdata = np.reshape(samples[n:n+wl,:],(wl,2,7)) # 400 * 2 * 7
                            temp.append(np.expand_dims(segdata, axis=0))
                        X.extend(temp)
                        Y.extend(i+np.ones(len(temp))) # relabel the class label to such as 1,2,3..
                        
                        # Segment data for rest
                        rest_start = index[-1] + rest_discard_n 
                        rest_end = rest_start + round(len(samples)/7)
                        # for class balance
                        if rest_end >= len(emg):
                           rest_end = len(emg)-1
                        rest_index = np.arange(rest_start, rest_end, dtype=int)
                        
                        samples_rest = emg[rest_index][:,valid_channel]
                        
                        temp = []
                        for n in range(0,samples_rest.shape[0]-wl, non_overlap_n):
                            segdata = np.reshape(samples_rest[n:n+wl,:],(wl,2,7)) # 400 * 2 * 7
                            temp.append(np.expand_dims(segdata, axis=0))
                        X.extend(temp)
                        Y.extend(np.zeros(len(temp))) # class label 0 for rest
...
if __name__ == "__main__":
    ...
    data_params['sb_list'] = [10] # subject list; 
    # data_params['sb_list'] = list(range(1, 11)) change it later
    data_params['class_list'] = [1, 3, 4, 6, 9, 10, 11] # class list
